...
secondS:.string "second"
		...
		//Setting up the base address for the first box
		//Base address of the second box
...
main:
	...
	add	x19,	fp,	first_s		//Calculates the base address of the first box
	add	x20,	fp,	second_s	//Calculates the base of the second box

	//Initialisation of boxes
	mov 	x8,		x19		//Moves the first address to the argument register
	bl		newBox						//Makes a box for the first box

	mov		x8,		x20		//Moves the first address to the argument register
	bl		newBox						//Makes a box for the first box
	...
	mov		x0,		x19		//Prints out the values for the first box
	adrp	x1,		firstS
	add		w1,		w1,		:lo12:firstS
	bl		printBox

	mov		x0,		x20		//Prints out the values for the second box
	adrp	x1,		secondS
	add		w1,		w1,		:lo12:secondS
	bl		printBox

	//Checking if boxes are the same. Changing them if they are
	mov		x0,		x19		//Moves the address of the first box to the argument register
	mov		x1,		x20		//Moves the second address to the following register
	...
	mov		x0,		x19		//Moves the address of the first box
	mov		w1,		-5					//Adds an x value
	mov		w2,		7					//Adds the y value
	bl		move						//Calls out the subroutine MOVE

	mov		x0,		x20		//Moves the address of teh second box
...
skip:
	...
	mov		x0,		x19		//Prints out the new first box values
	adrp	x1,		firstS
	add		w1,		w1,		:lo12:firstS
	bl		printBox

	mov		x0,		x20		//Prints out the new second box values
	adrp	x1,		secondS
	add		w1,		w1,		:lo12:secondS
	bl		printBox
...
//---------------------------END OF MAIN---------------------------//
	...
	//struct box newBox
			//Defines the x9 reg as the base address of the local variable
...
newBox:
	...
	add	x9,	fp,	b_s			//Calculates the base address of the local variable
	mov	w10,	1					//Moves 1 to a register

	str	xzr,	[x9,	box_Porigin + point_x]	//Stores zero in x
	str xzr,	[x9,	box_Porigin + point_y]	//Stores zero in y
	str w10,	[x9,	box_Dsize + dim_width]	//Stores 1 in width
	str w10,	[x9,	box_Dsize + dim_height]	//Stores 1 in height
	str w10,	[x9,	box_area]				//Stores 1 in area

	ldr	w10,	[x9,	box_Porigin + point_x]
	str	w10,	[x8,		box_Porigin + point_x]	//Stores the local x to the main x

	ldr w10,	[x9,	box_Porigin + point_y]
	str	w10,	[x8,		box_Porigin + point_y]	//Stores the local y to the main y

	ldr w10,	[x9,	box_Dsize + dim_width]
	str	w10,	[x8,		box_Dsize + dim_width]	//Stores the local width to the main width

	ldr w10,	[x9,	box_Dsize + dim_height]
